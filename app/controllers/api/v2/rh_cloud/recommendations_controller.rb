  ...
  module V2
    module RhCloud
      class RecommendationsController < ::Api::V2::BaseController
        ...
        api :GET, '/organizations/:organization_id/rh_cloud/recommendations', N_('List Insights recommendations')
        ...
        def show
          organization = Organization.find(params[:organization_id])
          forward_cloud_request("api/insights/v1/rule/#{params[:id]}", organization)
        end

        api :GET, '/organizations/:organization_id/hosts/:host_id/rh_cloud/recommendations', N_('List Insights recommendations for a host')
        param :organization_id, Integer, required: true, desc: N_("Set the current organization context for the request")
        ...
        def host_recommendations
          ...
          forward_cloud_request("api/insights/v1/system/#{insights_uuid}/reports/", organization)
        end
